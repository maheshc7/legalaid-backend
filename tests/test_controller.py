...
from app.controllers.controller import app as App
...
def test_upload_file_invalid_format(client):
    response = client.post("/upload", content_type="multipart/form-data", data={"file": (io.BytesIO(b"sample content"), "sample.txt")})
...
def test_upload_file_success(mock_parse_pdf, client):
    mock_parse_pdf.return_value = {
        ...
        "events": [{"id": "uuid", "subject": "Event", "date": "2023-07-19", "description": "Description"}],
        "length": 1
    }
    response = client.post("/upload", content_type="multipart/form-data", data={"file": (io.BytesIO(b"sample content"), "sample.pdf")})
